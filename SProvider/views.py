...
def serviceproviderlogin(request):
    ...
    return render(request,'Service_Provider/serviceproviderlogin.html')
...
def View_Prediction_Of_Cardiac_Arrest_Type_Ratio(request):
    ...
    return render(request, 'Service_Provider/View_Prediction_Of_Cardiac_Arrest_Type_Ratio.html', {'objs': obj})

def View_Remote_Users(request):
    obj=ClientRegister_Model.objects.all()
    return render(request,'Service_Provider/View_Remote_Users.html',{'objects':obj})

def ViewTrendings(request):
    topic = cardiac_arrest_prediction.objects.values('topics').annotate(dcount=Count('topics')).order_by('-dcount')
    return  render(request,'Service_Provider/ViewTrendings.html',{'objects':topic})

def charts(request,chart_type):
    chart1 = detection_ratio.objects.values('names').annotate(dcount=Avg('ratio'))
    return render(request,"Service_Provider/charts.html", {'form':chart1, 'chart_type':chart_type})

def charts1(request,chart_type):
    chart1 = detection_accuracy.objects.values('names').annotate(dcount=Avg('ratio'))
    return render(request,"Service_Provider/charts1.html", {'form':chart1, 'chart_type':chart_type})

def View_Prediction_Of_Cardiac_Arrest_Type(request):
    obj =cardiac_arrest_prediction.objects.all()
    return render(request, 'Service_Provider/View_Prediction_Of_Cardiac_Arrest_Type.html', {'list_objects': obj})

def likeschart(request,like_chart):
    charts =detection_accuracy.objects.values('names').annotate(dcount=Avg('ratio'))
    return render(request,"Service_Provider/likeschart.html", {'form':charts, 'like_chart':like_chart})
...
def train_model(request):
    ...
    return render(request,'Service_Provider/train_model.html', {'objs': obj})